...
import {
  ...
  updateUserDetail,
  userPaymentDetail,
} from "../../Store/UpdateUserData/UpdateUserDataSlice";
...
const UserDetail = () => {
  ...
  const userPaymentsData = async () => {
    try {
      dispatch(
        ...
        const findUserCircles = response.payload;
        console.log(findUserCircles, "Find User is coming");
        setCirclesData([...findUserCircles]);
      });
    } catch (error) {
      console.error("Error fetching users:", error);
    }
  ...
  const handleCircleStatus = (str) => {
    if (str === "active") {
      const filteredActiveCircles = circlesData.filter(
        (item) => item.statusId === 2
      );
      setFilterCircles(filteredActiveCircles);
      setCircleColor("active");
    } else {
      const filterCompletedCircles = circlesData.filter(
                ...
                <Accordion className="mt-4 box-sdw" key={index}>
                  ...
                  >
                    <tbody className="w-full flex justify-between ">
                      ...
                      <td className="font-fam-nunito w-14">
                        <Typography>{item?.numUsers}</Typography>
                      </td>
                      <td className="font-fam-nunito w-14">
                        <Typography>
                  ...
                  <AccordionDetails className="accord-detail">
                          ...
                          <tbody className="w-full flex flex-row justify-between">
                            <tr className="w-full flex flex-row justify-between">
                              <td className="font-fam-nunito w-25">
                                {new Date(
                                  item?.upcomingPayout?.endDate.substring(0, 10)
                        ...
                        <table className="w-full ">
                          ...
                          <tbody className="w-full flex flex-row justify-between">
                            <tr className="w-full flex flex-row justify-between">
                              <td className="font-fam-nunito w-25">
                                {new Date(
                                  item?.upcomingPayment?.endDate.substring(
                                    0,
                                    10
                                  )
                                ).toLocaleString("en-us", { month: "long" })}
                    ...
                    <div className="box-design-dynamic p-4 bg-[#F1F4F9]">
                      ...
                      {item?.previousPayments?.length !== 0 ? (
                        <thead className="w-full flex flex-row justify-between">
                          <th className="w-full flex flex-row justify-between">
                            <td className="font-fam-nunito header-head w-25">
                              Month
                      ...
                      {item?.previousPayments?.length !== 0 ? (
                        item?.previousPayments?.map((payment) =>
                          payment !== undefined ? (
                            <>
                              <tbody className="w-full flex justify-between items-center mt-imp">
                                <td className="font-fam-nunito w-25">
                                  <Typography>
                                    {payment?.endDate &&
                                      new Date(
                                        payment?.endDate.substring(0, 10)
